...
```{r}
cov_reg <- aov(qlogis(coverage_p) ~ Region + Outflow_Location + left + monkey, data = glyco_c)
summary(cov_reg)
r1 <- TukeyHSD(cov_reg, which = c("Region", "Outflow_Location"), conf.level = 0.95)
...
```{r}
plot(sfit_c2, pars=c("RegionControl High Flow","RegionControl Low Flow","RegionNon-Lasered","Outflow_LocationCorneaoscleral","Outflow_LocationESV","Outflow_LocationISV","Outflow_LocationSC","Outflow_LocationTM","Outflow_LocationUveal"), main = "Coverage Fixed Effects")

plot(sfit_c2, regex_pars = "^b\\[\\(Intercept)\\ Region", main = "Coverage Random Effects")
